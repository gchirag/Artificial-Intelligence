function [Y, R, E] = Isomap(D,names, intersecting, dataarg , n_fcn, n_size, options ); 
...
elseif nargin < 4
     options = struct('dims',1:10,'overlay',0,'comp',1,'display',1,'verbose',1); 
end
...
for i = 1:size(intersecting,2)              %% Setting images intersecting with obstacle as not reachable, ie. setting all their edge weights as infinite 
    D1(intersecting(1,i),:) = INF ;
    D1(:,intersecting(1,i)) = INF ;
end
...
%%%%%%%%%%%%%%%%%%%%%%%  Displays path from start to final image via the shortest path  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


for i = 1:size(path,2)
l=strcat(strcat('hw3_dataset/1_2d_3k/',names{path(i)}),'.png');
tmpimg=imread(l{1});
%fig=figure;
imshow(tmpimg,'InitialMagnification',67)
pause(0.5);
%saveas(fig,strcat('partgimages/',strcat(num2str(i),'.png')));
drawnow update;
end
...
if (displ==1)
     ...
     if ~isempty(twod)
         ...
         B = setdiff(A,intersecting);
         
         figure;
         hold on;
         
         plot3(Y.coords{twod}(1,B), Y.coords{twod}(2,B),Y.coords{twod}(3,B), 'ro');
         plot3(Y.coords{twod}(1,path),Y.coords{twod}(2,path),Y.coords{twod}(3,path),'-b*');
         box on;
         view(26,42);
